...
$(jQuery("#page_content").find(".data-row")).each( function(){
	try {
        if(this.childNodes[1].innerText.trim() !== 'Key Features'&& this.childNodes[1].innerText.trim() !== '' && this.childNodes[1].innerText.trim() !== undefined)
        {   
    ...
    for(let i=0;i<array_allversions_specs.length;i++)
    {
        ...
        for(let e=0;e<specifications.length;e++) //Specs
        {
            ...
            for(let j=0;j<specs.length;j++)
            {
               ...
               // if(specs[j]['value'].substring(0,specs[j]['value'].indexOf('[')).trim() === array_specs[m] || specs[j]['value'] === array_specs[m])//Specs
               if(specs[j]['customKey'] === array_specs_codes[m].dataset.sectionId)//Specs
                {  // console.log(specs[j]);
                    if(specs[j]['specType'] === 'technical')
                    {
                        //if(specs[j]['unit'].includes('MPG') || specs[j]['unit'].includes('mpg') ){}
                        //else
                        //{
                        found=true;
                            value=specs[j]['value'];
                    ...
                    else
                    {
                        ...
                        //if(specs[j]['specStatus']==="SYSTEMATIC")
                        {   found=true;
                            value='X';
                            if(i === array_allversions_specs.length-1)
                            {
                ...
                {
                   /* if(specs[j]['unit'] !== undefined ) //Extraer specs con valor numerico
                    {
                        if(specs[j]['unit'].includes('MPG') || specs[j]['unit'].includes('mpg') ){}
                        else
                        {
                            if(specs[j]['unit'].substring(0,specs[j]['unit'].indexOf('[')).trim() === array_specs[m] || specs[j]['unit'] === array_specs[m])
                            {
                            found=true;
                                value=specs[j]['value'];
                                if(i === array_allversions_specs.length-1)
       ...
       if(packs.length>=1|| value !== 'X' || value !== 'O') //packages specs
        {
            ...
            for(let j=0;j<packs.length;j++) //packs
            {
                for(let e=0;e<packs[j].specs.length;e++) //packs.specs
                {
                   ...
                   // if(packs[j].specs[e]['value'].substring(0,packs[j].specs[e]['value'].indexOf('[')).trim() === array_specs[m] || packs[j].specs[e]['value'] === array_specs[m]) 
       ...
       /* for(let e=0;e<consumptionSpecs.length;e++) //consumptionSpecs
        {
            
                if(consumptionSpecs[e]['unit'].includes(array_specs[m]))
                {   
                     found=true;
                        value=consumptionSpecs[e]['value'];
                        if(i === array_allversions_specs.length-1)
                        {
                            table=table+'</td><td>'+value+'</td></tr>';
                        }
    ...
    for(let i=0;i<array_allversions_specs.length;i++)
    {
        ...
        for(let e=0;e<accessoriesSpecs.length;e++) //Accessories
        {
            ...
            for(let j=0;j<specs.length;j++)
            {
                ...
                //if(specs[j]['value'].substring(0,specs[j]['value'].indexOf('[')).trim() === array_accessories[m] || specs[j]['value'] === array_accessories[m])
                if(specs[j]['customKey'].includes(array_accessories_codes[m]))
                {   //console.log(specs[j]);
                    if(specs[j]['type']==="SYSTEMATIC")
                    {   found=true;
                        value='X';
                        if(i === array_allversions_specs.length-1)
                        {
    ...
    for(let i=0;i<array_allversions_specs.length;i++)
    {
        ...
        if(packs.length>=1) //packages specs
        {
            ...
            for(let j=0;j<packs.length;j++) //packs
            {
                    ...
                    if(packs[j]['name'].substring(0,packs[j]['name'].indexOf('[')).trim().replace(/&nbsp;|\u00a0/g," ") === array_packages[m] || packs[j]['name'].replace(/&nbsp;|\u00a0/g," ") === array_packages[m] ) 
                    {   
